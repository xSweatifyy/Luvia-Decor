...
async function startServer() {
  ...
  app.post('/api/coupons',(req,res)=>{try{if(!requireAdminRole(req,res))return;const code=String(req.body?.code||'').trim().toUpperCase();const type=req.body?.type==='fixed'?'fixed':'percent';const value=Number(req.body?.value)||0;if(!code)return res.status(400).json({error:'Zadejte kód slevy.'});if(value<=0)return res.status(400).json({error:'Hodnota slevy musí být kladná.'});if(type==='percent'&&value>100)return res.status(400).json({error:'Procentní sleva může být nejvýše 100 %.'});res.status(201).json(db.addCoupon({id:`cup-${Date.now()}`,code,type,value,active:req.body?.active!==false,createdAt:new Date().toISOString(),note:req.body?.note||''}));}catch(err:any){res.status(500).json({error:err?.message||'Chyba vytvoření slevového kódu'});}});
  app.put('/api/coupons/:id',(req,res)=>{try{if(!requireAdminRole(req,res))return;const updates:any={};if(typeof req.body?.active==='boolean')updates.active=req.body.active;if(req.body?.value!==undefined){const value=Number(req.body.value)||0;if(value<=0)return res.status(400).json({error:'Hodnota slevy musí být kladná.'});updates.value=value;}if(req.body?.note!==undefined)updates.note=req.body.note;const updated=db.updateCoupon(req.params.id,updates);if(!updated)return res.status(404).json({error:'Slevový kód nenalezen.'});res.json(updated);}catch(err:any){res.status(500).json({error:err?.message||'Chyba aktualizace slevového kódu'});}});
  app.delete('/api/coupons/:id',(req,res)=>{try{if(!requireAdminRole(req,res))return;const deleted=db.deleteCoupon(req.params.id);if(!deleted)return res.status(404).json({error:'Slevový kód nenalezen.'});res.json({success:true});}catch(err:any){res.status(500).json({error:err?.message||'Chyba mazání slevového kódu'});}});
  app.post('/api/coupons/validate',(req,res)=>{try{const inputCode=String(req.body?.code||'').trim();const coupon=db.validateCoupon(inputCode);if(!coupon)return res.status(404).json({valid:false,error:'Slevový kód je neplatný nebo vypršel.'});res.json({valid:true,code:coupon.code,type:coupon.type,value:coupon.value,categoryIds:coupon.categoryIds||[]});}catch(err:any){res.status(500).json({error:err?.message||'Chyba ověření slevového kódu'});}});

  app.get('/api/gallery',(_req,res)=>{try{res.json(db.getGallery());}catch(err:any){res.status(500).json({error:err?.message});}});
  app.post('/api/gallery',(req,res)=>{try{res.status(201).json(db.addGalleryItem(req.body));}catch(err:any){res.status(500).json({error:err?.message});}});
  app.delete('/api/gallery/:id',(req,res)=>{try{res.json({success:db.deleteGalleryItem(req.params.id)});}catch(err:any){res.status(500).json({error:err?.message});}});

  app.post('/api/auth/login',(req,res)=>{try{const{email,password}=req.body;if(!email||!password)return res.status(400).json({error:'Zadejte e-mail a heslo.'});const user=db.verifyAdmin(email,password);if(!user)return res.status(401).json({error:'Neplatný e-mail nebo heslo.'});const token=`luvia_tok_${Buffer.from(`${user.email}:${Date.now()}`).toString('base64')}`;res.json({success:true,user,token});}catch(err:any){res.status(500).json({error:err?.message||'Chyba přihlášení'});}});
  app.get('/api/auth/users',(_req,res)=>{try{res.json(db.getAdminUsers());}catch(err:any){res.status(500).json({error:err?.message});}});
  app.post('/api/auth/users',(req,res)=>{try{const{email,name,role,password}=req.body;if(!email||!password)return res.status(400).json({error:'E-mail a heslo jsou povinné.'});res.status(201).json(db.createAdminUser({email,name:name||email.split('@')[0],role:role||'admin',password}));}catch(err:any){res.status(500).json({error:err?.message});}});
  app.delete('/api/auth/users/:id',(req,res)=>{try{const success=db.deleteAdminUser(req.params.id);if(!success)return res.status(400).json({error:'Uživatele nelze smazat nebo nebyl nalezen.'});res.json({success:true});}catch(err:any){res.status(500).json({error:err?.message});}});
  app.post('/api/auth/change-password',(req,res)=>{try{const{email,newPassword}=req.body;if(!email||!newPassword)return res.status(400).json({error:'Chybí e-mail nebo nové heslo.'});db.updatePassword(email,newPassword);res.json({success:true,message:'Heslo bylo úspěšně změněno.'});}catch(err:any){res.status(500).json({error:err?.message});}});
  app.post('/api/test-resend',async(req,res)=>{try{const{apiKey,targetEmail,fromEmail}=req.body;const keyToUse=apiKey||db.getConfig().resend?.apiKey;const target=targetEmail||db.getConfig().ordersEmail||'ondrej.andel@email.cz';res.json(await sendTestEmail(keyToUse,target,fromEmail));}catch(err:any){res.status(500).json({success:false,message:err?.message||'Chyba testu Resend'});}});
  ...
  app.listen(PORT,'0.0.0.0',()=>console.log(`🌿 Luvia Decor server running at http://0.0.0.0:${PORT}`));
}
startServer();

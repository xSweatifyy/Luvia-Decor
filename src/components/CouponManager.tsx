...
export const CouponManager: React.FC = () => {
  ...
  const effectiveCategories = useMemo<ProductCategory[]>(() => {
    ...
    const unique = new Map<string, ProductCategory>();
    products.forEach(p => {
      const name = String(p.category || '').trim();
      if (!name) return;
      const id = name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
      if (id) unique.set(id, { id, name });
    });
    return [...unique.values()].sort((a,b)=>a.name.localeCompare(b.name,'cs'));
  }, [categories, products]);

  const categoryMap = useMemo(() => new Map(effectiveCategories.map(category => [category.id, category.name])), [effectiveCategories]);
  const categoryNameMap = useMemo(() => new Map(effectiveCategories.map(category => [category.name, category.id])), [effectiveCategories]);
  ...
  const createCoupon=async()=>{
    ...
    try{
      const r=await fetch('/api/coupons',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
        code:clean,type,value:amount,active:true,
        // Product.category is stored as the category name. Persist the names
        // so the server-side order validation can match the same value.
        categoryIds:selectedCategoryNames,note:''
      })});

...
export const CartPromoCode: React.FC = () => {
  ...
  useEffect(() => { const originalFetch = window.fetch.bind(window); window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => { const url = typeof input === 'string' ? input : input instanceof URL ? input.toString() : input.url; if (url.includes('/api/orders') && init?.method?.toUpperCase() === 'POST') { const promoCode = localStorage.getItem('luvia_cart_promo'); if (promoCode) { try { const body = JSON.parse(String(init.body || '{}')); body.couponCode = promoCode; init = { ...init, body: JSON.stringify(body) }; } catch {} } } return originalFetch(input, init); }; return () => { window.fetch = originalFetch; }; }, []);
  ...
  async function validate(raw: string, notify = true) {
    ...
    try {
      ...
      const r = await fetch(`/api/coupons?action=validate&code=${encodeURIComponent(clean)}`, { method:'GET', cache:'no-store' });

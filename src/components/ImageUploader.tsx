    ...
    <div className={`space-y-2 ${className}`}>
        ...
        <div className="relative inline-block">
          ...
          {preview && (
            <button
              ...
              className="absolute -top-2 -right-2 p-1 bg-rose-500 text-white rounded-full shadow-sm hover:bg-rose-600 transition"
            >
              <X className="w-3 h-3" />
            </button>
          )}
      ...
      <div
        ...
        className={`relative border-2 border-dashed rounded-xl p-4 text-center transition-colors ${
          dragActive
            ? 'border-[#8C7355] bg-[#FAF5EE]'
            : 'border-[#E3DACF] bg-[#FAF8F5] hover:border-[#C5A880]'
        }`}
      >
        <input
          ref={fileInputRef}
        ...
        <div className="space-y-2 pointer-events-none">
          {isUploading ? (
            <Loader2 className="w-6 h-6 text-[#8C7355] mx-auto animate-spin" />
          ) : preview ? (
            <ImageIcon className="w-6 h-6 text-emerald-600 mx-auto" />
          ) : (
            <Upload className="w-6 h-6 text-[#8C7355] mx-auto" />
          )}
          ...
          <div className="text-xs text-[#5C5046]">
            {isUploading ? (
              ...
              <span className="text-emerald-700">Soubor připraven. Klikněte na Nahrát.</span>
            ) : (
              <>
                <span className="font-semibold">Přetáhněte soubor sem</span>
                <span className="block text-[10px] text-[#7B6E63] mt-0.5">
                  nebo klikněte pro výběr • JPEG, PNG, WebP, SVG (max 5 MB)
                </span>
              </>
            )}
          </div>

        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-8 pb-12 border-b border-[#382F28]">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-14 h-14 rounded-xl overflow-hidden border border-[#C5A880]/40 shadow-md shrink-0"><SafeImage src="/Luvia-Decor.jpeg" alt="Logo Luvia Decor" className="w-full h-full" loading="lazy" /></div>
              <div><span className="font-editorial text-2xl tracking-[0.2em] font-bold text-[#FAF6F0] uppercase block leading-tight">{config.logoText || config.siteName}</span><p className="text-xs text-[#C5A880] tracking-wider uppercase font-medium mt-0.5">Květinový ateliér & dekorace</p></div>
            </div>
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2.5 text-xs">
              <li><button onClick={() => handleNav('home')} className="hover:text-[#FAF6F0] hover:translate-x-1 transition cursor-pointer">Domovská stránka</button></li>
              <li><button onClick={() => handleNav('catalog')} className="hover:text-[#FAF6F0] hover:translate-x-1 transition cursor-pointer">Celá nabídka & E-shop</button></li>
              <li><button onClick={() => handleNav('custom-order')} className="hover:text-[#FAF6F0] hover:translate-x-1 transition cursor-pointer text-[#C5A880] font-medium">Zakázková tvorba na míru ✨</button></li>
              <li><button onClick={() => handleNav('gallery')} className="hover:text-[#FAF6F0] hover:translate-x-1 transition cursor-pointer">Inspirace & Galerie</button></li>
              <li><button onClick={() => handleNav('contact')} className="hover:text-[#FAF6F0] hover:translate-x-1 transition cursor-pointer">Kontakt & Osobní odběr</button></li>
            </ul>
          </div>
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-3 text-xs text-[#B8ACA0]">
              <li className="flex items-start gap-2.5"><Mail className="w-4 h-4 text-[#C5A880] shrink-0 mt-0.5" /><div><span className="block text-[10px] uppercase text-[#8F8174]">Objednávky</span><a href={`mailto:${config.ordersEmail}`} className="text-[#FAF6F0] hover:underline font-medium">{config.ordersEmail}</a></div></li>

...
export const OrderTrackingManager: React.FC = () => {
  ...
  const load = useCallback(async (refreshTracking = false) => {
    ...
    try {
      ...
      setOrders(prev => list.map(serverOrder =>
        savingIds.current.has(serverOrder.id)
  ...
  return <section className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E8DFC8] shadow-sm space-y-5">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#F2ECE4] pb-4"><div className="flex items-center gap-3"><div className="w-11 h-11 rounded-2xl bg-[#FAF5EE] border border-[#E8DFC8] flex items-center justify-center text-[#8C7355]"><Truck className="w-5 h-5" /></div><div><h2 className="font-editorial text-2xl font-bold text-[#2D2723]">Sledování zásilek</h2><p className="text-xs text-[#7B6E63]">Stav zásilek se automaticky kontroluje každou minutu.</p></div></div><button type="button" onClick={() => load(false)} disabled={loading} className="px-3 py-2 bg-[#FAF5EE] border border-[#E3DACF] rounded-xl text-xs font-semibold flex items-center gap-2"><RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} />Obnovit</button></div>

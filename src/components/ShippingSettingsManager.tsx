...
const DEFAULT_SHIPPING: ShippingConfig = {
  carriers: {
    DPD: { enabled: true, address: 105, pickup_point: 75 },
    'Zásilkovna': { enabled: true, address: 89, pickup_point: 62 },
  },
  personalPickup: {
    enabled: true,
    price: 0,
    label: 'Osobní odběr – Kroměříž',
  },
};
...
const normalize = (value: unknown): ShippingConfig => {
  ...
  const rawCarriers =
    source?.carriers && typeof source.carriers === 'object'
      ? source.carriers
  ...
  Object.entries(rawCarriers).forEach(([name, cfg]) => {
    ...
    carriers[name] = {
      ...
      address: Number(c?.address ?? 0),
      pickup_point: Number(c?.pickup_point ?? 0),
    };
  });
...
export const ShippingSettingsManager: React.FC = () => {
  ...
  const [shipping, setShipping] = useState<ShippingConfig>(() =>
    normalize(config?.shipping),
  );
  const [saving, setSaving] = useState(false);
  const [newCarrier, setNewCarrier] = useState('');

  useEffect(() => {
    setShipping(normalize(config?.shipping));
  }, [config?.shipping]);

  const updateCarrier = (
    name: string,
    patch: Partial<ShippingCarrierConfig>,
  ) => {
    setShipping((prev) => ({
      ...prev,
      carriers: {
  ...
  const addCarrier = () => {
    ...
    setShipping((prev) => ({
      ...
      carriers: {
        ...prev.carriers,
        [name]: { enabled: true, address: 0, pickup_point: 0 },
      },
    }));
    setNewCarrier('');
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('Nastavení dopravy se nepodařilo uložit.');
      }
      ...
      addToast(
        'success',
        'Doprava uložena',
        'Přepravci, jejich aktivita a ceny byly aktualizovány.',
      );
    } catch (error: unknown) {
      addToast(
        'error',
        ...
        error instanceof Error
          ? error.message
          : 'Nastavení dopravy se nepodařilo uložit.',
      );
    } finally {
      setSaving(false);
    }
    ...
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="bg-white rounded-3xl border border-[#E8DFC8] shadow-sm overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-[#7B6E63] mt-1 max-w-2xl">
                Přidávejte, upravujte, aktivujte/deaktivujte nebo mažte přepravce a nastavujte ceny na adresu a na výdejní místo.
              </p>
            </div>
            <button
              type="button"
        ...
        <div className="p-6 sm:p-8 space-y-6">
          <div className="grid gap-4">
            {Object.entries(shipping.carriers).map(([name, carrier]) => (
              <div
                key={name}
                className="rounded-2xl border border-[#E3DACF] p-5 bg-white"
              >
                <div className="flex items-center justify-between gap-3 mb-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-[#817469]">
                        Přepravce pro objednávky
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="inline-flex items-center gap-2 text-xs font-semibold text-[#5C5046]">
                      <input
                        ...
                        onChange={(e) =>
                          updateCarrier(name, { enabled: e.target.checked })
                        }
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <label className="text-xs font-semibold text-[#5C5046]">
                    Na adresu
                    <span className="block text-[10px] font-normal text-[#8A7D72] mb-1.5">
                      Cena v Kč
                    </span>
                    {input(carrier.address, (n) =>
                      updateCarrier(name, { address: n }),
                    )}
                  </label>
                  <label className="text-xs font-semibold text-[#5C5046]">
                    Výdejní místo
                    ...
                    {input(carrier.pickup_point, (n) =>
                      updateCarrier(name, { pickup_point: n }),
                    )}
                  </label>
                </div>
              </div>
          ...
          <div className="rounded-2xl border border-[#E3DACF] p-5 bg-[#FAF8F5]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <label className="text-xs font-semibold text-[#5C5046]">
                ...
                {input(shipping.personalPickup.price, (n) =>
                  setShipping((prev) => ({
                    ...prev,
                    personalPickup: {
                      ...prev.personalPickup,
          ...
          <div className="pt-2 border-t border-[#EFE7DE]">
            <div className="font-bold text-[#2D2723] mb-3">
              Přidat dalšího přepravce
            </div>
        ...
        <div className="px-6 sm:px-8 py-4 bg-[#FAF8F5] border-t border-[#EFE7DE] flex items-center gap-2 text-[11px] text-[#7B6E63]">
          ...
          Nastavení se ukládá do společné konfigurace e-shopu a je dostupné zákazníkům.
        </div>
      </div>
    </section>
  );

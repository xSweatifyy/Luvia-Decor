...
export const ShippingSettingsManager: React.FC = () => {
  ...
  useEffect(() => {
    // The parent polls the config periodically. Never replace unsaved local
    // edits with that background response.
  ...
  const updateCarrier = (name: string, patch: Partial<ShippingCarrierConfig>) => {
    updateShipping(prev => ({
      ...prev,
      carriers: { ...prev.carriers, [name]: { ...prev.carriers[name], ...patch } },
    }));
  };
  ...
  const addCarrier = () => {
    ...
    updateShipping(prev => ({
      ...prev,
      carriers: { ...prev.carriers, [name]: { enabled: true, address: 0, pickup_point: 0, box: 0 } },
    }));
    setNewCarrier('');
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok || !updated || !updated.shipping) {
        throw new Error(updated?.error || `Server odmítl uložení nastavení dopravy (HTTP ${res.status}).`);
      }

      // Verify the persisted value before clearing the dirty flag.
      const verifyRes = await fetch('/api/config', { cache: 'no-store', headers: { 'Cache-Control': 'no-cache' } });
      if (!verifyRes.ok) throw new Error('Nastavení bylo odesláno, ale nepodařilo se ověřit jeho uložení.');
      const verified = await verifyRes.json();
      const saved = JSON.stringify(normalize(verified.shipping));
      const expected = JSON.stringify(payload);
      if (saved !== expected) throw new Error('Server vrátil jiné nastavení dopravy, než bylo uloženo.');

      dirtyRef.current = false;
      updateConfigState(verified);
      setShipping(normalize(verified.shipping));
      ...
      <div className="bg-white rounded-3xl border border-[#E8DFC8] shadow-sm overflow-hidden">
        <div className="p-6 sm:p-8 border-b border-[#EFE7DE] bg-[#FAF7F2]">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="inline-flex items-center gap-2 text-[#8C7355] text-xs font-bold uppercase tracking-wider"><Truck className="w-4 h-4" /> Doprava a přepravci</div>
              <h2 className="font-editorial text-2xl sm:text-3xl font-bold text-[#2D2723] mt-2">Nastavení dopravy</h2>
              <p className="text-sm text-[#7B6E63] mt-1 max-w-2xl">Přidávejte, upravujte, aktivujte/deaktivujte nebo mažte přepravce a nastavujte ceny na adresu, výdejní místo a box.</p>
            </div>
        ...
        <div className="p-6 sm:p-8 space-y-6">
            ...
            {Object.entries(shipping.carriers).map(([name, carrier]) => (
              <div key={name} className="rounded-2xl border border-[#E3DACF] p-5 bg-white">
                <div className="flex items-center justify-between gap-3 mb-5">
                  <div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-xl flex items-center justify-center ${carrier.enabled ? 'bg-emerald-50 text-emerald-700' : 'bg-stone-100 text-stone-400'}`}><Truck className="w-5 h-5" /></div><div><div className="font-bold text-[#2D2723]">{name}</div><div className="text-[11px] text-[#817469]">Přepravce pro objednávky</div></div></div>
                  <div className="flex items-center gap-2"><label className="inline-flex items-center gap-2 text-xs font-semibold text-[#5C5046]"><input type="checkbox" checked={carrier.enabled} onChange={e => updateCarrier(name, { enabled: e.target.checked })} className="accent-[#8C7355]" /> Aktivní</label><button type="button" onClick={() => removeCarrier(name)} className="p-2 text-stone-400 hover:text-rose-600 cursor-pointer" title="Smazat přepravce"><Trash2 className="w-4 h-4" /></button></div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <label className="text-xs font-semibold text-[#5C5046]">Na adresu<span className="block text-[10px] font-normal text-[#8A7D72] mb-1.5">Cena v Kč</span>{input(carrier.address, n => updateCarrier(name, { address: n }))}</label>
                  <label className="text-xs font-semibold text-[#5C5046]">Výdejní místo<span className="block text-[10px] font-normal text-[#8A7D72] mb-1.5">Cena v Kč</span>{input(carrier.pickup_point, n => updateCarrier(name, { pickup_point: n }))}</label>
                  <label className="text-xs font-semibold text-[#5C5046]">Box<span className="block text-[10px] font-normal text-[#8A7D72] mb-1.5">Cena v Kč</span>{input(carrier.box, n => updateCarrier(name, { box: n }))}</label>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-2xl border border-[#E3DACF] p-5 bg-[#FAF8F5]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="text-xs font-semibold text-[#5C5046]">Název<span className="block text-[10px] font-normal text-[#8A7D72] mb-1.5">Zobrazí se zákazníkovi</span><input required value={shipping.personalPickup.label} onChange={e => updateShipping(prev => ({ ...prev, personalPickup: { ...prev.personalPickup, label: e.target.value } }))} className="w-full px-3 py-2.5 bg-white border border-[#E3DACF] rounded-xl text-sm" /></label>
              <label className="text-xs font-semibold text-[#5C5046]">Cena v Kč<span className="block text-[10px] font-normal text-[#8A7D72] mb-1.5">Obvykle 0 Kč</span>{input(shipping.personalPickup.price, n => updateShipping(prev => ({ ...prev, personalPickup: { ...prev.personalPickup, price: n } })))}</label>
            </div>
          </div>

...
export const OrderStatusCategory: React.FC = () => {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      setOrders(data);
      setDrafts(Object.fromEntries(data.map((order: Order) => [order.id, order.status])));
    } catch (error: any) {
  ...
  const pendingCount = useMemo(() => orders.filter(order => order.status === 'nova').length, [orders]);
  ...
  const saveStatus = async (order: Order) => {
    ...
    try {
      let res = await fetch(`/api/orders/${order.id}/status`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        res = await fetch('/api/order-status', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (!res.ok) {
        let message = 'Aktualizace stavu selhala.';
        try {
          const data = await res.json();
      ...
      setDrafts(prev => ({ ...prev, [order.id]: order.status }));
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E8DFC8] shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-[#7B6E63] mt-1">Změna stavu objednávky se provádí pouze zde. Zákazníkovi se při změně odešle e-mail.</p>
          </div>
          <div className="flex items-center gap-2">
            {pendingCount > 0 && (
              <span className="px-3 py-1.5 rounded-full bg-amber-100 text-amber-800 text-[11px] font-bold">{pendingCount} nových</span>
            )}
            <button
              onClick={loadOrders}
              disabled={loading}
              className="px-3 py-2 bg-[#FAF5EE] hover:bg-[#F2ECE4] text-xs font-semibold rounded-xl border border-[#E3DACF] flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
            >
              ...
              <div key={order.id} className="p-5 sm:p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="px-2.5 py-1 rounded-full bg-[#FAF5EE] border border-[#E3DACF] text-[10px] font-bold text-[#6B5C4F]">
                      {statusLabel(order.status)}
                    </span>
                  </div>
                  <p className="text-xs text-[#5C5046] mt-1">{order.customer?.fullName || 'Zákazník'} · {order.customer?.email || 'Bez e-mailu'}</p>
                  <p className="text-[11px] text-stone-500 mt-0.5">{new Date(order.createdAt).toLocaleString('cs-CZ')}</p>
                </div>

                <div className="flex items-center gap-2 w-full lg:w-auto">
                  <select
                    value={drafts[order.id] || order.status}
                    onChange={(event) => setDrafts(prev => ({ ...prev, [order.id]: event.target.value as Order['status'] }))}
                    className="flex-1 lg:w-56 px-3 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs font-semibold cursor-pointer"
                  >
                  ...
                  <button
                    ...
                    disabled={savingId === order.id || (drafts[order.id] || order.status) === order.status}
                    className="px-3.5 py-2.5 bg-[#2D2723] hover:bg-[#8C7355] text-white rounded-xl text-xs font-bold flex items-center gap-1.5 cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    <Save className="w-3.5 h-3.5" />
                    {savingId === order.id ? 'Ukládám…' : 'Uložit'}
                  </button>
                </div>
              </div>
            ))}

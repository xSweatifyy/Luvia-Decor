...
const designs = [
  ['obsidian', 'Obsidian Gold'], ['champagne', 'Champagne Atelier'], ['ivory', 'Ivory Pearl'], ['emerald', 'Emerald Botanical'],
  ['velvet', 'Burgundy Velvet'], ['rose', 'Rose Champagne'], ['midnight', 'Midnight Luxe'], ['mocha', 'Dark Mocha'],
  ['sage', 'Sage Garden'], ['navy', 'Navy & Champagne'], ['blackwhite', 'Black & Ivory'], ['lavender', 'Lavender Silk'],
];
...
export const GiftVoucherManager: React.FC = () => {
  ...
  const [design, setDesign] = useState('obsidian');
  ...
  const load = async () => {
    ...
    try {
      ...
      setVouchers(Array.isArray(d) ? d.filter((x: Coupon) => x.note === 'gift-voucher' && x.code !== 'LUVIA-ZQR5-Q7VF') : []);
      if (t.ok) {
  ...
  const createPdf = async (cardCode: string, amount: number, security: string) => {
    try {
      await createGiftCardPdf({
        amount,
        cardCode,
        securityCode: security,
        slogan: (config as any).slogan || 'Ručně tvořené dekorace a květinový ateliér v Kroměříži',
        logoText: (config as any).logoText || (config as any).siteName || 'LUVIA DECOR',
        design,
      });
    } catch (e: any) {
      addToast('error', 'PDF dárkové karty', e?.message || 'PDF se nepodařilo vytvořit.');
    }
  };
  ...
  const create = async () => {
    ...
    if (!c || !Number.isFinite(a) || a < MIN) return addToast('error', 'Chybí údaje', `Kód a částka od ${MIN} Kč jsou povinné.`);
    setBusy('create');
    try {
      const r = await fetch('/api/coupons', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code: c, type: 'fixed', value: a, active: true, note: 'gift-voucher', categoryIds: [] }),
      });
      const d = await r.json().catch(() => null);
      if (!r.ok) throw new Error(d?.error || 'Dárkovou kartu se nepodařilo vytvořit.');
      await createPdf(d.code || c, a, d.securityCode || '');
      setCode(''); setValue(''); setPaid(false);
      await load();
      addToast('success', 'Karta vytvořena a PDF uloženo', `${c} · ${money(a)} Kč`);
    } catch (e: any) {
      addToast('error', 'Dárková karta', e?.message || 'Chyba.');
    } finally { setBusy(null); }
  };

  const downloadExistingPdf = async (v: Coupon) => {
    try {
      const r = await fetch(`/api/gift-card?action=secret&code=${encodeURIComponent(v.code)}`, { cache: 'no-store' });
      const d = await r.json();
      if (!r.ok || !d?.securityCode) throw new Error('Bezpečnostní kód karty se nepodařilo načíst.');
      await createGiftCardPdf({
        amount: Number(v.remainingValue ?? v.value), cardCode: v.code, securityCode: d.securityCode,
        slogan: (config as any).slogan || 'Ručně tvořené dekorace a květinový ateliér v Kroměříži',
        logoText: (config as any).logoText || (config as any).siteName || 'LUVIA DECOR', design,
      });
      addToast('success', 'PDF vytvořeno', v.code);
    } catch (e: any) {
      addToast('error', 'PDF', e?.message || 'Chyba.');
    }
  };
  ...
  const recharge = async (v: Coupon) => {
    ...
    if (raw === null) return;
    const a = Number(raw);
    if (!Number.isFinite(a) || a < MIN) return addToast('error', 'Neplatná částka', `Dobití musí být alespoň ${MIN} Kč.`);
    setBusy(v.id);
    try {
      const r = await fetch('/api/coupons?action=admin-adjust-balance', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: v.id, delta: a }),
      });
      const d = await r.json().catch(() => null);
      if (!r.ok) throw new Error(d?.error || 'Dobití se nepodařilo.');
      await load();
      addToast('success', 'Karta dobita', `${v.code} + ${money(a)} Kč`);
    } catch (e: any) { addToast('error', 'Dobití', e?.message || 'Chyba.'); }
    finally { setBusy(null); }
  };
  ...
  const toggle = async (v: Coupon) => {
    setBusy(v.id);
    try {
      const r = await fetch('/api/coupons?action=admin-set-active', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: v.id, active: !v.active }),
      });
      const d = await r.json().catch(() => null);
      if (!r.ok) throw new Error(d?.error || 'Stav se nepodařilo změnit.');
      setVouchers(p => p.map(x => x.id === v.id ? { ...x, ...d } : x));
      addToast('success', 'Stav změněn', d.active ? 'Karta je aktivní.' : 'Karta je neaktivní.');
    } catch (e: any) { addToast('error', 'Stav karty', e?.message || 'Chyba.'); }
    finally { setBusy(null); }
  };

  const remove = async (v: Coupon) => {
    if (!confirm(`Opravdu smazat kartu ${v.code}?`)) return;
    setBusy(v.id);
    try {
      const r = await fetch(`/api/coupons?id=${encodeURIComponent(v.id)}`, { method: 'DELETE' });
      if (!r.ok) throw new Error('Kartu se nepodařilo smazat.');
      setVouchers(p => p.filter(x => x.id !== v.id));
    } catch (e: any) { addToast('error', 'Dárková karta', e?.message || 'Chyba.'); }
    finally { setBusy(null); }
  };

  const confirmTopup = async (t: Topup) => {
    setBusy(t.id);
    try {
      const r = await fetch('/api/coupons?action=confirm-topup', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: t.id }),
      });
      const d = await r.json().catch(() => null);
      if (!r.ok) throw new Error(d?.error || 'Dobití se nepodařilo potvrdit.');
      await load();
      addToast('success', 'Dobití potvrzeno', `${t.code} + ${money(t.amount)} Kč`);
    } catch (e: any) { addToast('error', 'Dobití', e?.message || 'Chyba.'); }
    finally { setBusy(null); }
  };
  ...
  const cancelTopup = async (t: Topup) => {
    setBusy(t.id);
    try {
      const r = await fetch('/api/coupons?action=cancel-topup', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: t.id }),
      });
  ...
  return <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
    <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E8DFC8] shadow-sm space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-3"><div className="w-11 h-11 rounded-2xl bg-[#FAF3E8] flex items-center justify-center text-[#8C7355]"><Gift className="w-5 h-5" /></div><div><h2 className="font-editorial text-2xl font-bold text-[#2D2723]">Dárkové karty</h2><p className="text-xs text-[#7B6E63]">Luxusní PDF, česká diakritika, zůstatky, dobíjení a stav karty.</p></div></div>
        <button type="button" onClick={load} className="px-3 py-2 rounded-xl border text-xs font-bold flex items-center gap-2"><RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />Obnovit</button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-5 gap-3 p-5 rounded-2xl bg-[#FAF6F0] border border-[#E3DACF]">
        <div><label className="block text-[11px] font-semibold mb-1">Kód karty *</label><div className="flex gap-2"><input value={code} onChange={e => setCode(e.target.value.toUpperCase())} placeholder="LUVIA-AB12-CD34" className="min-w-0 flex-1 px-3 py-2.5 bg-white border border-[#E3DACF] rounded-xl text-xs font-bold" /><button type="button" onClick={() => setCode(makeCode())} className="px-3 rounded-xl border bg-white"><Shuffle className="w-4 h-4" /></button></div></div>
        <div><label className="block text-[11px] font-semibold mb-1">Částka *</label><input type="number" min="200" step="0.01" value={value} onChange={e => setValue(e.target.value)} placeholder="500" className="w-full px-3 py-2.5 bg-white border border-[#E3DACF] rounded-xl text-xs" /></div>
        <div><label className="block text-[11px] font-semibold mb-1"><Palette className="w-3 h-3 inline mr-1" />Vzhled</label><select value={design} onChange={e => setDesign(e.target.value)} className="w-full px-3 py-2.5 bg-white border border-[#E3DACF] rounded-xl text-xs font-semibold">{designs.map(d => <option key={d[0]} value={d[0]}>{d[1]}</option>)}</select></div>
        <label className="flex items-center gap-2 text-[11px] font-semibold self-end pb-2"><input type="checkbox" checked={paid} onChange={e => setPaid(e.target.checked)} />Objednávka je zaplacená</label>

export type PageRoute = 'home' | 'catalog' | 'custom-order' | 'gallery' | 'contact' | 'cart' | 'terms' | 'privacy' | 'admin' | 'gift-card' | 'gift-card-balance' | 'complaint';
...
export interface MaintenanceBannerConfig { enabled:boolean; variant:'maintenance'|'warning'|'info'|'closed'; title:string; message:string; until?:string; }

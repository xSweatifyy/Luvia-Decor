export type PageRoute = 'home' | 'catalog' | 'custom-order' | 'gallery' | 'contact' | 'cart' | 'terms' | 'privacy' | 'admin' | 'gift-card' | 'gift-card-balance' | 'complaint' | 'account';
...
export interface CustomerAccount { id:string; email:string; name:string; phone:string; createdAt:string; lastLogin?:string; addresses:CustomerAddress[]; favorites:string[]; collections:CustomerCollection[]; preferences:CustomerPreferences; }

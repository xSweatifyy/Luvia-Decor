...
export const GiftCardPage: React.FC = () => {
  ...
  const verify = async () => {
    ...
    try {
      ...
      if (!r.ok || !d?.valid || !d?.giftVoucher || Number(d.remainingValue) <= 0) throw new Error(d?.error || 'Dárková karta není platná nebo nemá zůstatek.');
      setBalance(d);
      addToast('success', 'Dárková karta ověřena', `Aktuální zůstatek: ${money(d.remainingValue)} Kč.`);
    } catch (e: any) {
      setBalance(null);
      addToast('error', 'Ověření se nepodařilo', e?.message || 'Zkontrolujte údaje karty.');
  ...
  const addGiftCard = () => {
    ...
    addToCart(product, 1, `Dárková karta · ${money(purchaseAmount)} Kč`);
    setPage('cart');
  };
  ...
  const addTopup = () => {
    ...
    addToCart(product, 1, `Dobití dárkové karty · karta ${balance.code} · bezpečnostní kód ${securityCode.trim().toUpperCase()} · ${money(purchaseAmount)} Kč`);
    setPage('cart');
  };
  ...
  const downloadPreviewPdf = async () => {
    ...
    try {
      ...
      addToast('error', 'PDF dárkové karty se nepodařilo vytvořit', e?.message || 'Zkuste to znovu.');
    }
  };

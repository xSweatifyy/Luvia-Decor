...
export const GiftCardPage: React.FC = () => {
  ...
  const downloadPreviewPdf = async () => {
    ...
    // Do not create an intermediate canvas here. A zero-sized canvas can make
    // jsPDF/browser image processing throw `createPattern` errors. Load the
    // image as a real HTMLImageElement and pass it directly to jsPDF only after
    // it has a valid natural size.
    try {
      const img = new Image();
      img.decoding = 'async';
      ...
      if (img.naturalWidth > 0 && img.naturalHeight > 0) {
        pdf.addImage(img, 'JPEG', 15, 14, 28, 28, undefined, 'FAST');
      }
    } catch {
      // The PDF remains fully usable without the optional logo image.
    }

    pdf.setTextColor(250,246,240); pdf.setFont('helvetica','bold'); pdf.setFontSize(22); pdf.text('LUVIA DECOR',52,27);
    pdf.setFontSize(8); pdf.setTextColor(217,196,168); pdf.text('KVĚTINOVÝ ATELIÉR & DEKORACE',52,34);
    pdf.setTextColor(250,246,240); pdf.setFontSize(25); pdf.text('DÁRKOVÁ KARTA',105,76,{align:'center'});
    pdf.setFontSize(30); pdf.text(`${money(purchaseAmount)} Kč`,105,96,{align:'center'});
    pdf.setFontSize(10); pdf.setTextColor(225,216,205); pdf.text(config.slogan||'S láskou tvořeno v Kroměříži',105,109,{align:'center'});
    pdf.setFontSize(8); pdf.text('Digitální karta · doručení e-mailem · kód a bezpečnostní kód obdržíte po zpracování objednávky.',105,124,{align:'center',maxWidth:175});

  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (res.ok && data?.valid) {
        ...
        addToast('success', 'Slevový kód použit', data.type === 'percent' ? `Sleva ${Number(data.value) || 0} % z ceny objednávky.` : `Sleva ${(Number(data.value) || 0).toLocaleString('cs-CZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Kč z ceny objednávky.`);
        return;
      }
      ...
      if (fallback) {
        ...
        addToast('success', 'Slevový kód použit', type === 'percent' ? `Sleva ${value} % z ceny objednávky.` : `Sleva ${value.toLocaleString('cs-CZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Kč z ceny objednávky.`);
        return;
      }
    ...
    <div id="cart-page" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-5"><div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E8DFC8] shadow-md space-y-6"><h2 className="font-editorial text-2xl font-bold text-[#2D2723] border-b border-[#F2ECE4] pb-3">Doručení a objednávka</h2><form onSubmit={handleSubmitOrder} className="space-y-4">
          ...
          <div className="space-y-3"><label className="block text-xs font-semibold text-[#5C5046]">1. Vyberte dopravce *</label><div className="grid grid-cols-1 sm:grid-cols-2 gap-2">{activeCarriers.map(([name, cc]) => <button key={name} type="button" onClick={() => chooseCarrier(name)} className={`p-3 rounded-xl border text-left ${carrier === name && deliveryMethod !== 'personal_pickup' ? 'border-[#8C7355] bg-[#FAF5EE] ring-1 ring-[#8C7355]/20' : 'border-[#E3DACF] bg-[#FAF8F5]'}`}><div className="text-xs font-bold text-[#2D2723]">{name}</div><div className="text-[10px] text-[#817469] mt-1">Adresa od {Number(cc?.address || 0).toLocaleString('cs-CZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Kč · výdejní místo od {Number(cc?.pickup_point || 0).toLocaleString('cs-CZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Kč</div></button>)}{shippingConfig?.personalPickup?.enabled !== false && <button type="button" onClick={() => { setDeliveryMethod('personal_pickup'); setPickupPoint(''); }} className={`p-3 rounded-xl border text-left sm:col-span-2 ${deliveryMethod === 'personal_pickup' ? 'border-[#8C7355] bg-[#FAF5EE] ring-1 ring-[#8C7355]/20' : 'border-[#E3DACF] bg-[#FAF8F5]'}`}><div className="text-xs font-bold">{shippingConfig?.personalPickup?.label || 'Osobní odběr – Kroměříž'}</div><div className="text-[10px] text-[#817469] mt-1">{Number(shippingConfig?.personalPickup?.price || 0) === 0 ? 'Zdarma' : `${Number(shippingConfig?.personalPickup?.price || 0).toLocaleString('cs-CZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Kč`}</div></button>}</div></div>
          {deliveryMethod !== 'personal_pickup' && <>
            <div className="space-y-3"><label className="block text-xs font-semibold text-[#5C5046]">2. Zvolte způsob doručení *</label><div className="grid grid-cols-1 sm:grid-cols-3 gap-2">{([['address','Na adresu','Kurýr'],['pickup_point','Výdejní místo','Pobočka'],['box','Box','Výdejní box']] as const).map(([value,label,desc]) => <button key={value} type="button" onClick={() => chooseDelivery(value)} className={`p-3 rounded-xl border text-left ${deliveryMethod === value ? 'border-[#8C7355] bg-[#FAF5EE] ring-1 ring-[#8C7355]/20' : 'border-[#E3DACF] bg-[#FAF8F5]'}`}><div className="text-xs font-bold">{label}</div><div className="text-[10px] text-[#817469] mt-1">{desc} · {(value === 'address' ? Number(carrierConfig?.address || 0) : value === 'pickup_point' ? Number(carrierConfig?.pickup_point || 0) : Number(carrierConfig?.box ?? carrierConfig?.pickup_point ?? 0)).toLocaleString('cs-CZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Kč</div></button>)}</div></div>
          ...
          <div className="pt-4 border-t border-[#F2ECE4] space-y-2 text-xs"><div className="flex justify-between"><span>Mezisoučet produktů:</span><span>{cartTotal.toLocaleString('cs-CZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Kč</span></div><div className="flex justify-between"><span>Doprava:</span><span className={shipping > 0 ? 'font-semibold' : 'text-emerald-700 font-semibold'}>{shipping > 0 ? `${shipping.toLocaleString('cs-CZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Kč` : 'Zdarma'}</span></div>{discount > 0 && <div className="flex justify-between text-emerald-700 font-semibold"><span>Sleva ({appliedCoupon?.code}):</span><span>−{discount.toLocaleString('cs-CZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Kč</span></div>}<div className="flex justify-between items-baseline pt-2 border-t border-[#F0EAE1] text-base font-bold"><span>Celková cena:</span><span className="text-xl text-[#8C7355]">{grandTotal.toLocaleString('cs-CZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} Kč</span></div></div>

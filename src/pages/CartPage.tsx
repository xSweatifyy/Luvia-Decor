...
export const CartPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const emailAddress = order?.customer?.email || email;
    const referenceId = String(order?.orderNumber || order?.id || '');
    if (!order || !emailAddress || !referenceId || trustpilotSentRef.current === referenceId) return;
    ...
    const sendTrustpilotInvitation = () => {
      ...
      if (typeof tp === 'function') {
        ...
        try {
          tp('createInvitation', {
            ...
            recipientName: order.customer?.fullName || fullName || '',
            referenceId,
            source: 'InvitationScript',
            productSkus: items.map((item: any) => String(item.productId || item.sku || '')).filter(Boolean)
          });
        } catch (error) {
      ...
      if (attempts++ < 50) window.setTimeout(sendTrustpilotInvitation, 200);
    };
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      const res = await fetch('/api/coupons/validate', {
        ...
        body: JSON.stringify({
          code,
          items: cart.map(item => ({ productId: item.product.id, category: item.product.category, price: item.product.price, quantity: item.quantity }))
        })
      });
      ...
      if (!res.ok || !data?.valid) {
        ...
        if (fbCoupon) {
          ...
          setAppliedCoupon({ code: String(fbCoupon.code).toUpperCase(), type, value, categoryIds: [] });
      ...
      addToast('success', 'Slevový kód použit', Array.isArray(data.categoryIds) && data.categoryIds.length > 0 ? 'Sleva se vztahuje pouze na vybrané kategorie v košíku.' : (data.type === 'percent' ? `Sleva ${(Number(data.value) || 0)}% z ceny objednávky.` : `Sleva ${(Number(data.value) || 0).toLocaleString('cs-CZ')} Kč z ceny objednávky.`));
    } catch {
      setAppliedCoupon(null);
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        customer: { fullName, email, phone, street, city, zip, country: 'Česká republika', note },
        items: cart.map(item => ({ productId: item.product.id, title: item.product.title, price: item.product.price, quantity: item.quantity, category: item.product.category, imageUrl: item.product.imageUrl, customNote: item.customNote })),
      ...
      clearCart();
      try { confetti({ particleCount: 80, spread: 70, origin: { y: 0.6 }, colors: ['#C5A880', '#2D2723', '#8C7355', '#EBDCC8'] }); } catch { /* ignore */ }
    ...
    return (
      <div id="cart-success-view" className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="bg-white rounded-3xl p-8 sm:p-12 border border-[#E8DFC8] shadow-lg text-center space-y-8">
          <div className="w-20 h-20 rounded-full bg-[#F4F9F4] text-emerald-600 flex items-center justify-center mx-auto border-2 border-[#D3E8D6] shadow-sm"><CheckCircle2 className="w-10 h-10" /></div>
          <div className="space-y-3">
            <span className="text-xs font-bold uppercase tracking-[0.25em] text-[#8C7355]">Číslo objednávky: {orderCompleted.orderNumber || 'LUV-2026'}</span>
            <h1 className="font-editorial text-3xl sm:text-4xl lg:text-5xl font-bold text-[#2D2723] leading-tight">Objednávka byla úspěšně přijata. Již brzy Vás budeme kontaktovat.</h1>
            <p className="text-sm text-[#7B6E63] max-w-lg mx-auto leading-relaxed">Děkujeme za Váš nákup v ateliéru Luvia Decor.</p>
          </div>
          <div className="bg-[#FAF8F5] p-6 rounded-2xl border border-[#EDE5DA] text-left text-xs text-[#5C5046] space-y-2 max-w-md mx-auto">
            <p className="font-bold text-[#2D2723] uppercase text-[11px] border-b border-[#E8DFC8] pb-2 mb-2">Detaily předání & doručení:</p>
          ...
          <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-4">
            <button onClick={() => { setOrderCompleted(null); setPage('catalog'); }} className="w-full sm:w-auto px-8 py-3.5 bg-[#2D2723] hover:bg-[#8C7355] text-white text-xs font-bold uppercase tracking-wider rounded-full shadow-md transition cursor-pointer">Pokračovat v prohlížení e-shopu</button>
            <button onClick={() => { setOrderCompleted(null); setPage('home'); }} className="w-full sm:w-auto px-6 py-3.5 bg-[#FAF5EE] hover:bg-[#F2ECE4] text-[#2D2723] text-xs font-semibold rounded-full border border-[#E3DACF] transition cursor-pointer">Zpět na hlavní stránku</button>
          </div>
          <p className="text-[11px] text-[#8C7355] pt-4">V případě dotazů volejte naši linku {config.phoneDisplay} nebo pište na {config.ordersEmail}</p>
        </div>
      </div>
    );
  }
  ...
  if (cart.length === 0) {
    return (
      <div id="cart-empty-view" className="max-w-2xl mx-auto px-4 py-20 text-center space-y-6">
        ...
        <div className="space-y-2"><h1 className="font-editorial text-3xl sm:text-4xl font-bold text-[#2D2723]">Váš košík je prázdný</h1><p className="text-sm text-[#7B6E63] max-w-md mx-auto">Zatím jste do košíku nevložili žádnou dekoraci. Prohlédněte si naši nabídku věnců, aranžmá a doplňků.</p></div>
  ...
  return (
    <div id="cart-page" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-5"><div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E8DFC8] shadow-md space-y-6"><h2 className="font-editorial text-2xl font-bold text-[#2D2723] border-b border-[#F2ECE4] pb-3">Doručovací & kontaktní údaje</h2><form onSubmit={handleSubmitOrder} className="space-y-4"><div><label className="block text-xs font-semibold text-[#5C5046] mb-1">Jméno a příjmení *</label><input type="text" required value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Např. Kateřina Dvořáková" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" /></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-3"><div><label className="block text-xs font-semibold text-[#5C5046] mb-1">E-mail pro potvrzení *</label><input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="katerina@seznam.cz" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" /></div><div><label className="block text-xs font-semibold text-[#5C5046] mb-1">Telefon *</label><input type="tel" required value={phone} onChange={e => setPhone(e.target.value)} placeholder="+420 777 123 456" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" /></div></div><div><label className="block text-xs font-semibold text-[#5C5046] mb-1">Ulice a číslo popisné</label><input type="text" value={street} onChange={e => setStreet(e.target.value)} placeholder="Kovářská 15" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" /></div><div className="grid grid-cols-2 gap-3"><div><label className="block text-xs font-semibold text-[#5C5046] mb-1">Město</label><input type="text" value={city} onChange={e => setCity(e.target.value)} placeholder="Kroměříž" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" /></div><div><label className="block text-xs font-semibold text-[#5C5046] mb-1">PSČ</label><input type="text" value={zip} onChange={e => setZip(e.target.value)} placeholder="767 01" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" /></div></div><div><label className="block text-xs font-semibold text-[#5C5046] mb-1">Poznámka k objednávce</label><textarea rows={2} value={note} onChange={e => setNote(e.target.value)} placeholder="Např. osobní vyzvednutí v ateliéru / speciální přání k balení..." className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" /></div><div className="space-y-2"><label className="block text-xs font-semibold text-[#5C5046]">Slevový kód</label>{appliedCoupon ? (<div className="flex items-center justify-between gap-2 px-3.5 py-2.5 bg-emerald-50 border border-emerald-200 rounded-xl"><span className="flex items-center gap-2 text-xs font-bold text-emerald-700"><Tag className="w-4 h-4" />{appliedCoupon.code}<span className="font-semibold">(−{appliedCoupon.type === 'percent' ? `${appliedCoupon.value} %` : `${appliedCoupon.value.toLocaleString('cs-CZ')} Kč`})</span></span><button type="button" onClick={() => setAppliedCoupon(null)} className="p-1 text-emerald-700 hover:text-rose-600 transition cursor-pointer" aria-label="Odebrat slevový kód" title="Odebrat slevový kód"><X className="w-4 h-4" /></button></div>) : (<div className="flex gap-2"><input type="text" value={couponInput} onChange={e => setCouponInput(e.target.value.toUpperCase())} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); handleApplyCoupon(); } }} placeholder="Např. JARO10" className="flex-1 px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] uppercase tracking-wide focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" /><button type="button" onClick={handleApplyCoupon} disabled={couponChecking} className="px-4 py-2.5 bg-[#FAF5EE] hover:bg-[#F2ECE4] border border-[#E3DACF] text-[#5C4830] text-xs font-bold rounded-xl transition cursor-pointer disabled:opacity-50 whitespace-nowrap">{couponChecking ? 'Ověřuji…' : 'Použít'}</button></div>)}</div><div className="pt-4 border-t border-[#F2ECE4] space-y-2 text-xs"><div className="flex justify-between text-[#5C5046]"><span>Mezisoučet produktů:</span><span>{cartTotal.toLocaleString('cs-CZ')} Kč</span></div><div className="flex justify-between text-[#5C5046]"><span>Doprava:</span><span className="text-emerald-700 font-semibold">Kroměříž a okolí / Osobní odběr</span></div>{discount > 0 && <div className="flex justify-between text-emerald-700 font-semibold"><span>Sleva ({appliedCoupon?.code}):</span><span>−{discount.toLocaleString('cs-CZ')} Kč</span></div>}{appliedCoupon && appliedCoupon.categoryIds.length > 0 && <p className="text-[10px] text-[#8C7355]">Kód platí pouze na vybrané kategorie. Sleva se započítává jen z oprávněných položek.</p>}<div className="flex justify-between items-baseline pt-2 border-t border-[#F0EAE1] text-base font-bold text-[#2D2723]"><span>Celková cena:</span><span className="text-xl text-[#8C7355]">{grandTotal.toLocaleString('cs-CZ')} Kč</span></div></div><button type="submit" disabled={submitting} className="w-full py-4 px-6 bg-[#2D2723] hover:bg-[#8C7355] text-white text-xs font-bold uppercase tracking-wider rounded-xl shadow-lg transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 active:scale-98"><Send className="w-4 h-4" /><span>{submitting ? 'Odesílám objednávku...' : 'Odeslat objednávku (bez nutnosti platby)'}</span></button></form></div></div>
      </div>
    </div>
  );
};
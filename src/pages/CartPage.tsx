...
import { Trash2, Plus, Minus, ShoppingBag, ArrowLeft, CheckCircle2, Truck, Send, Tag, X } from 'lucide-react';
...
export const CartPage: React.FC = () => {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      setAppliedCoupon({ code: data.code, type: data.type, value: data.value, categoryIds });
      addToast('success', 'Slevový kód použit', categoryIds.length > 0 ? 'Sleva se vztahuje pouze na vybrané kategorie v košíku.' : (data.type === 'percent' ? `Sleva ${data.value} % z ceny objednávky.` : `Sleva ${data.value.toLocaleString('cs-CZ')} Kč z ceny objednávky.`));
    } catch {
      setAppliedCoupon(null);
  ...
  const handleSubmitOrder = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        items: cart.map(item => ({ productId: item.product.id, title: item.product.title, category: item.product.category, price: item.product.price, quantity: item.quantity, imageUrl: item.product.imageUrl, customNote: item.customNote })),
          ...
          <div className="space-y-3">
            {cart.map(item => (
              <div key={item.product.id} className="bg-white rounded-2xl p-4 sm:p-5 border border-[#E8DFC8] shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4 flex-1"><div className="w-20 h-20 rounded-xl overflow-hidden border border-[#EBE3D8] shrink-0"><SafeImage src={item.product.imageUrl} alt={item.product.title} className="w-full h-full" loading="lazy" /></div><div><h3 className="font-editorial text-base sm:text-lg font-bold text-[#2D2723] line-clamp-1">{item.product.title}</h3><p className="text-xs font-bold text-[#8C7355] mt-0.5">{item.product.price > 0 ? `${item.product.pricePrefix || ''} ${item.product.price.toLocaleString('cs-CZ')} Kč / ks`.trim() : 'Cena dle dohody (nezávazná poptávka)'}</p>{item.customNote && <p className="text-[11px] text-stone-500 italic mt-1">Pozn.: {item.customNote}</p>}</div></div>
                <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-4"><div className="flex items-center border border-[#E3DACF] rounded-xl bg-[#FAF8F5] p-0.5"><button onClick={() => updateCartQuantity(item.product.id, item.quantity - 1)} className="w-7 h-7 rounded-lg hover:bg-[#EBE2D7] text-xs font-bold flex items-center justify-center transition cursor-pointer"><Minus className="w-3 h-3" /></button><span className="w-8 text-center text-xs font-bold text-[#2D2723]">{item.quantity}</span><button onClick={() => updateCartQuantity(item.product.id, item.quantity + 1)} className="w-7 h-7 rounded-lg hover:bg-[#EBE2D7] text-xs font-bold flex items-center justify-center transition cursor-pointer"><Plus className="w-3 h-3" /></button></div><div className="text-right min-w-[90px]"><span className="text-sm font-bold text-[#2D2723]">{item.product.price > 0 ? `${(item.product.price * item.quantity).toLocaleString('cs-CZ')} Kč` : 'Dle dohody'}</span></div><button onClick={() => removeFromCart(item.product.id)} className="p-2 text-stone-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition cursor-pointer" title="Odebrat položku"><Trash2 className="w-4 h-4" /></button></div>
              </div>
            ))}
          </div>

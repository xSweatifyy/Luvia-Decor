...
export const CartPage: React.FC = () => {
  ...
  // Coupon / discount code state. Empty categoryIds means the code applies to all products.
  ...
  const eligibleCouponSubtotal = appliedCoupon && appliedCoupon.categoryIds.length > 0
    ? cart.reduce((sum, item) => {
        if (!appliedCoupon.categoryIds.includes(item.product.category)) return sum;
      ...
      <div id="cart-success-view" className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="bg-white rounded-3xl p-8 sm:p-12 border border-[#E8DFC8] shadow-lg text-center space-y-8">
          ...
          <div className="pt-4 flex flex-col sm:flex-row items-center justify-center gap-4">
            <button onClick={() => { setOrderCompleted(null); setPage('catalog'); }} className="w-full sm:w-auto px-8 py-3.5 bg-[#2D2723] hover:bg-[#8C7355] text-white text-xs font-bold uppercase tracking-wider rounded-full shadow-md transition cursor-pointer">
              Pokračovat v prohlížení e-shopu
            </button>
            <button onClick={() => { setOrderCompleted(null); setPage('home'); }} className="w-full sm:w-auto px-6 py-3.5 bg-[#FAF5EE] hover:bg-[#F2ECE4] text-[#2D2723] text-xs font-semibold rounded-full border border-[#E3DACF] transition cursor-pointer">
    ...
    <div id="cart-page" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        <div className="lg:col-span-7 space-y-4">
            ...
            {cart.map((item) => (
              <div key={item.product.id} className="bg-white rounded-2xl p-4 sm:p-5 border border-[#E8DFC8] shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4 flex-1">
                  <div className="w-20 h-20 rounded-xl overflow-hidden border border-[#EBE3D8] shrink-0">
                    <SafeImage src={item.product.imageUrl} alt={item.product.title} className="w-full h-full" loading="lazy" />
                  </div>
                  <div>
                    <h3 className="font-editorial text-base sm:text-lg font-bold text-[#2D2723] line-clamp-1">{item.product.title}</h3>
                    <p className="text-xs font-bold text-[#8C7355] mt-0.5">{item.product.price > 0 ? `${item.product.pricePrefix || ''} ${item.product.price.toLocaleString('cs-CZ')} Kč / ks`.trim() : 'Cena dle dohody (nezávazná poptávka)'}</p>
                    {item.customNote && <p className="text-[11px] text-stone-500 italic mt-1">Pozn.: {item.customNote}</p>}
                  </div>
                </div>
                <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-4">
                  <div className="flex items-center border border-[#E3DACF] rounded-xl bg-[#FAF8F5] p-0.5">
                    <button onClick={() => updateCartQuantity(item.product.id, item.quantity - 1)} className="w-7 h-7 rounded-lg hover:bg-[#EBE2D7] text-xs font-bold flex items-center justify-center transition cursor-pointer" aria-label="Snížit množství"><Minus className="w-3 h-3" /></button>
                    <span className="w-8 text-center text-xs font-bold text-[#2D2723]">{item.quantity}</span>
                    <button onClick={() => updateCartQuantity(item.product.id, item.quantity + 1)} className="w-7 h-7 rounded-lg hover:bg-[#EBE2D7] text-xs font-bold flex items-center justify-center transition cursor-pointer" aria-label="Zvýšit množství"><Plus className="w-3 h-3" /></button>
                  </div>
          ...
          <div className="bg-[#FAF6F0] rounded-2xl p-5 border border-[#E3DACF] space-y-2 text-xs text-[#5C5046]">
            <div className="flex items-center gap-2 text-[#8C7355] font-semibold"><Truck className="w-4 h-4" /><span>Doručení: Kroměříž a okolí nebo osobní převzetí v ateliéru</span></div>
            <p className="text-[11px] text-[#7B6E63]">Objednávka je nezávazná bez okamžité platby kartou. Před doručením či předáním vás budeme kontaktovat a potvrdíme přesný termín i adresu předání v Kroměříži a okolí.</p>
          </div>
        </div>

        <div className="lg:col-span-5">
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E8DFC8] shadow-md space-y-6">
            ...
            <form onSubmit={handleSubmitOrder} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-[#5C5046] mb-1">Jméno a příjmení *</label>
                <input type="text" required value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Např. Kateřina Dvořáková" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-[#5C5046] mb-1">E-mail pro potvrzení *</label>
                  <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="katerina@seznam.cz" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[#5C5046] mb-1">Telefon *</label>
                  <input type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+420 777 123 456" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-[#5C5046] mb-1">Ulice a číslo popisné</label>
                <input type="text" value={street} onChange={(e) => setStreet(e.target.value)} placeholder="Kovářská 15" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-[#5C5046] mb-1">Město</label>
                  <input type="text" value={city} onChange={(e) => setCity(e.target.value)} placeholder="Kroměříž" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[#5C5046] mb-1">PSČ</label>
                  <input type="text" value={zip} onChange={(e) => setZip(e.target.value)} placeholder="767 01" className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-[#5C5046] mb-1">Poznámka k objednávce</label>
                <textarea rows={2} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Např. osobní vyzvednutí v ateliéru / speciální přání k balení..." className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" />
              </div>
                ...
                {appliedCoupon ? (
                  <div className="flex items-center justify-between gap-2 px-3.5 py-2.5 bg-emerald-50 border border-emerald-200 rounded-xl">
                    <span className="flex items-center gap-2 text-xs font-bold text-emerald-700">
                      ...
                      {appliedCoupon.code}
                      <span className="font-semibold">(−{appliedCoupon.type === 'percent' ? `${appliedCoupon.value} %` : `${appliedCoupon.value.toLocaleString('cs-CZ')} Kč`})</span>
                    </span>
                  ...
                  <div className="flex gap-2">
                    <input type="text" value={couponInput} onChange={(e) => setCouponInput(e.target.value.toUpperCase())} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleApplyCoupon(); } }} placeholder="Např. JARO10" className="flex-1 px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] uppercase tracking-wide focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30" />
                    <button type="button" onClick={handleApplyCoupon} disabled={couponChecking} className="px-4 py-2.5 bg-[#FAF5EE] hover:bg-[#F2ECE4] border border-[#E3DACF] text-[#5C4830] text-xs font-bold rounded-xl transition cursor-pointer disabled:opacity-50 whitespace-nowrap">{couponChecking ? 'Ověřuji…' : 'Použít'}</button>
                  </div>
                )}
              </div>
              ...
              <div className="pt-4 border-t border-[#F2ECE4] space-y-2 text-xs">
                ...
                {discount > 0 && (
                  <div className="flex justify-between text-emerald-700 font-semibold">
                    <span>Sleva ({appliedCoupon?.code}):</span><span>−{discount.toLocaleString('cs-CZ')} Kč</span>
                  </div>
                )}
                {appliedCoupon && appliedCoupon.categoryIds.length > 0 && (
                  <p className="text-[10px] text-[#8C7355]">Kód platí pouze na vybrané kategorie. Sleva se započítává jen z oprávněných položek.</p>
                )}

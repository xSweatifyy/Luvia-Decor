  ...
  if (cart.length === 0) {
    return (
      <div id="cart-empty-view" className="max-w-2xl mx-auto px-4 py-20 text-center space-y-6">
        <div className="w-20 h-20 rounded-full bg-[#FAF5EE] text-[#8C7355] flex items-center justify-center mx-auto border border-[#E8DFC8]">
          <ShoppingBag className="w-10 h-10" />
        </div>
        <div className="space-y-2">
          <h1 className="font-editorial text-3xl sm:text-4xl font-bold text-[#2D2723]">
            Váš košík je prázdný
          </h1>
          <p className="text-sm text-[#7B6E63] max-w-md mx-auto">
            Zatím jste do košíku nevložili žádnou dekoraci. Prohlédněte si naši nabídku věnců, aranžmá a doplňků.
          </p>
        </div>
        <button
    ...
    <div id="cart-page" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      ...
      <div className="flex items-center justify-between border-b border-[#E8DFC8] pb-6">
        <div>
          <button
            onClick={() => setPage('catalog')}
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#8C7355] hover:underline mb-2 cursor-pointer"
          >
            <ArrowLeft className="w-3.5 h-3.5" />
            Zpět k výběru dekorací
          </button>
          <h1 className="font-editorial text-3xl sm:text-4xl font-bold text-[#2D2723]">
        ...
        <button
          onClick={clearCart}
          className="text-xs text-stone-500 hover:text-rose-600 transition cursor-pointer"
        >
          Vyprázdnit košík
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        ...
        <div className="lg:col-span-7 space-y-4">
          <h2 className="text-xs font-bold uppercase tracking-wider text-[#8C7355]">
            Vybrané dekorace ({cart.length})
          </h2>
          ...
          <div className="space-y-3">
            {cart.map((item) => (
              <div
                key={item.product.id}
                className="bg-white rounded-2xl p-4 sm:p-5 border border-[#E8DFC8] shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
              >
                {/* Product image & Title */}
                <div className="flex items-center gap-4 flex-1">
                  ...
                  <div>
                    <h3 className="font-editorial text-base sm:text-lg font-bold text-[#2D2723] line-clamp-1">
                      {item.product.title}
                    </h3>
                    <p className="text-xs font-bold text-[#8C7355] mt-0.5">
                      {item.product.price > 0
                        ? `${item.product.pricePrefix || ''} ${item.product.price.toLocaleString('cs-CZ')} Kč / ks`.trim()
                        : 'Cena dle dohody (nezávazná poptávka)'}
                    </p>
                    {item.customNote && (
                      <p className="text-[11px] text-stone-500 italic mt-1">
                        Pozn.: {item.customNote}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-4">
                  ...
                  <div className="flex items-center border border-[#E3DACF] rounded-xl bg-[#FAF8F5] p-0.5">
                    <button
                      ...
                      className="w-7 h-7 rounded-lg hover:bg-[#EBE2D7] text-xs font-bold flex items-center justify-center transition cursor-pointer"
                      aria-label="Snížit množství"
                    >
                      <Minus className="w-3 h-3" />
                    </button>
                    <span className="w-8 text-center text-xs font-bold text-[#2D2723]">
                      {item.quantity}
                    </span>
                    <button
                      onClick={() => updateCartQuantity(item.product.id, item.quantity + 1)}
          ...
          <div className="bg-[#FAF6F0] rounded-2xl p-5 border border-[#E3DACF] space-y-2 text-xs text-[#5C5046]">
            <div className="flex items-center gap-2 text-[#8C7355] font-semibold">
              <Truck className="w-4 h-4" />
              <span>Doručení: Kroměříž a okolí nebo osobní převzetí v ateliéru</span>
            </div>
            <p className="text-[11px] text-[#7B6E63]">
              Objednávka je nezávazná bez okamžité platby kartou. Před doručením či předáním vás budeme kontaktovat a potvrdíme přesný termín i adresu předání v Kroměříži a okolí.
            </p>
          </div>
        </div>

        {/* Right Column: Checkout Form without Payment */}
        <div className="lg:col-span-5">
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#E8DFC8] shadow-md space-y-6">

            <h2 className="font-editorial text-2xl font-bold text-[#2D2723] border-b border-[#F2ECE4] pb-3">
              Doručovací & kontaktní údaje
            </h2>
            ...
            <form onSubmit={handleSubmitOrder} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Např. Kateřina Dvořáková"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="katerina@seznam.cz"
                    className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30"
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Např. osobní vyzvednutí v ateliéru / speciální přání k balení..."
                  className="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30"
                />
              </div>
                ...
                {appliedCoupon ? (
                  <div className="flex items-center justify-between gap-2 px-3.5 py-2.5 bg-emerald-50 border border-emerald-200 rounded-xl">
                    <span className="flex items-center gap-2 text-xs font-bold text-emerald-700">
                      ...
                      <span className="font-semibold">
                        (−{appliedCoupon.type === 'percent' ? `${appliedCoupon.value} %` : `${appliedCoupon.value.toLocaleString('cs-CZ')} Kč`})
                      </span>
                    </span>
                    <button
                      type="button"
                      onClick={() => setAppliedCoupon(null)}
                      className="p-1 text-emerald-700 hover:text-rose-600 transition cursor-pointer"
                      aria-label="Odebrat slevový kód"
                      title="Odebrat slevový kód"
                    >
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E3DACF] rounded-xl text-xs text-[#2D2723] uppercase tracking-wide focus:outline-none focus:ring-2 focus:ring-[#8C7355]/30"
                    />
                    <button
                      type="button"
                      onClick={handleApplyCoupon}
                      disabled={couponChecking}
                      className="px-4 py-2.5 bg-[#FAF5EE] hover:bg-[#F2ECE4] border border-[#E3DACF] text-[#5C4830] text-xs font-bold rounded-xl transition cursor-pointer disabled:opacity-50 whitespace-nowrap"
                    >
              ...
              <div className="pt-4 border-t border-[#F2ECE4] space-y-2 text-xs">
                ...
                <div className="flex justify-between text-[#5C5046]">
                  <span>Doprava:</span>
                  <span className="text-emerald-700 font-semibold">Kroměříž a okolí / Osobní odběr</span>
                </div>
                {discount > 0 && (
                ...
                <div className="flex justify-between items-baseline pt-2 border-t border-[#F0EAE1] text-base font-bold text-[#2D2723]">
                  <span>Celková cena:</span>
                  <span className="text-xl text-[#8C7355]">{grandTotal.toLocaleString('cs-CZ')} Kč</span>
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full py-4 px-6 bg-[#2D2723] hover:bg-[#8C7355] text-white text-xs font-bold uppercase tracking-wider rounded-xl shadow-lg transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 active:scale-98"
              >
                <Send className="w-4 h-4" />
                <span>{submitting ? 'Odesílám objednávku...' : 'Odeslat objednávku (bez nutnosti platby)'}</span>
              </button>
            </form>

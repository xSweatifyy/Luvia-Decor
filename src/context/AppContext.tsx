...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateProductItem = async (id: string, updates: Partial<Product>): Promise<Product> => {
    ...
    const merged: Product = {
      ...
      isPriceFrom: updates.isPriceFrom !== undefined ? Boolean(updates.isPriceFrom) : current.isPriceFrom,
    ...
    if (!response.ok) throw new Error((await response.json().catch(() => null))?.error || 'Produkt se nepodařilo upravit na serveru.');

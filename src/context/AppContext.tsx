...
const routeFromLocation = (): PageRoute => {
  ...
  const pathRoutes: Record<string, PageRoute> = {'/':'home','/home':'home','/eshop':'catalog','/e-shop':'catalog','/catalog':'catalog','/zakazkova-tvorba':'custom-order','/custom-order':'custom-order','/galerie':'gallery','/gallery':'gallery','/kontakt':'contact','/contact':'contact','/kosik':'cart','/cart':'cart','/obchodni-podminky':'terms','/terms':'terms','/admin':'admin'};
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const deleteProductItem = async (id:string) => { await deleteProductFromFirestore(id); try{await fetch(`/api/products/${encodeURIComponent(id)}`,{method:'DELETE'});}catch{} setProducts(prev=>prev.filter(x=>x.id!==id)); setCart(prev=>prev.filter(item=>item.product.id!==id)); return true; };

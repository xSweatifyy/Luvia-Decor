...
function removeUndefined<T>(value: T): T {
  if (Array.isArray(value)) return value.map(removeUndefined).filter(v => v !== undefined) as T;
  if (value && typeof value === 'object') {
    const result: Record<string, unknown> = {};
    for (const [key, child] of Object.entries(value as Record<string, unknown>)) if (child !== undefined) result[key] = removeUndefined(child);
    return result as T;
  }
  return value;
}
...
async function seedFirestoreIfNeeded(): Promise<void> {
  try {
    ...
    if (!metaSnap.exists()) {
      ...
      if (!configSnap.exists()) await setDoc(doc(db, CONFIG_COL, CONFIG_DOC), removeUndefined(initialSiteConfig));
      const gallerySnap = await getDocs(collection(db, GALLERY_COL));
      if (gallerySnap.empty) { const batch = writeBatch(db); for (const item of initialGallery) batch.set(doc(db, GALLERY_COL, item.id), removeUndefined(item)); await batch.commit(); }
      const reviewsSnap = await getDocs(collection(db, REVIEWS_COL));
      if (reviewsSnap.empty) { const batch = writeBatch(db); for (const review of initialReviews) batch.set(doc(db, REVIEWS_COL, review.id), removeUndefined(review)); await batch.commit(); }
      const adminSnap = await getDocs(collection(db, ADMIN_USERS_COL));
      if (adminSnap.empty) { const batch = writeBatch(db); for (const admin of initialAdminUsers) batch.set(doc(db, ADMIN_USERS_COL, admin.id), removeUndefined(admin)); await batch.commit(); }
      await setDoc(metaRef, { initialized: true, seededAt: new Date().toISOString() });
    }
  } catch (err) { console.error('Error initializing Firestore data:', err); }
}
...
export function subscribeGallery(callback:(items:GalleryItem[])=>void):()=>void{return onSnapshot(collection(db,GALLERY_COL),snapshot=>{const items:GalleryItem[]=[];snapshot.forEach(d=>items.push({...d.data() as GalleryItem,id:d.id}));callback(items);},err=>console.error('Firestore gallery subscription error:',err));}
export async function saveGalleryItemToFirestore(item:GalleryItem):Promise<GalleryItem>{const id=item.id||`gal-${Date.now()}`;const data=removeUndefined({...item,id});await setDoc(doc(db,GALLERY_COL,id),data,{merge:true});return data;}
...
export function subscribeCoupons(callback:(coupons:Coupon[])=>void):()=>void{return onSnapshot(collection(db,COUPONS_COL),snapshot=>{const coupons:Coupon[]=[];snapshot.forEach(d=>coupons.push({...d.data() as Coupon,id:d.id}));callback(coupons);},err=>console.error('Firestore coupons subscription error:',err));}
export async function saveCouponToFirestore(coupon:Coupon):Promise<Coupon>{const id=coupon.id||`cup-${Date.now()}`;const data=removeUndefined({...coupon,id});await setDoc(doc(db,COUPONS_COL,id),data,{merge:true});return data;}
export async function deleteCouponFromFirestore(id:string):Promise<boolean>{try{await deleteDoc(doc(db,COUPONS_COL,id));return true;}catch(err){console.error('Failed to delete coupon in firestore:',err);return false;}}
export async function findCouponByCodeInFirestore(code:string):Promise<Coupon|null>{try{const clean=String(code||'').trim().toUpperCase();if(!clean)return null;const snap=await getDocs(collection(db,COUPONS_COL));let found:Coupon|null=null;snap.forEach(d=>{const data=d.data() as Coupon;if(!found&&String(data.code||'').trim().toUpperCase()===clean&&data.active!==false)found={...data,id:d.id};});return found;}catch(err){console.error('Failed to find coupon in firestore:',err);return null;}}

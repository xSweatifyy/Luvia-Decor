...
/** Firestore rejects undefined values. Remove them recursively before writing. */
  ...
  try {
    ...
    if (!metaSnap.exists()) {
      ...
      if (gallerySnap.empty) {
        ...
        for (const item of initialGallery) batch.set(doc(db, GALLERY_COL, item.id), removeUndefined(item));
        await batch.commit();
      }
      const reviewsSnap = await getDocs(collection(db, REVIEWS_COL));
      if (reviewsSnap.empty) {
        const batch = writeBatch(db);
        for (const review of initialReviews) batch.set(doc(db, REVIEWS_COL, review.id), removeUndefined(review));
        await batch.commit();
      }
      const adminSnap = await getDocs(collection(db, ADMIN_USERS_COL));
      if (adminSnap.empty) {
        const batch = writeBatch(db);
        for (const admin of initialAdminUsers) batch.set(doc(db, ADMIN_USERS_COL, admin.id), removeUndefined(admin));
        await batch.commit();
      }
...
export async function createOrderInFirestore(order: Order): Promise<Order> {
  ...
  const data = removeUndefined({ ...order, id, createdAt: order.createdAt || new Date().toISOString() });
...
export async function saveGalleryItemToFirestore(item: GalleryItem): Promise<GalleryItem> {
  ...
  const data = removeUndefined({ ...item, id });

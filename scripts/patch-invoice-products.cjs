...
source = source.replace(
  ...
  "type InvoiceItem = { productId?: string; name: string; qty: number; price: number; vat: number };\ntype ShopProduct = { id: string | number; title?: string; name?: string; price?: number; inStock?: boolean; category?: string };"
);
source = source.replace(
...
source = source.replace(
  ...
  "  const setItem = (idx: number, key: keyof InvoiceItem, value: string | number) => setForm(f => ({ ...f, items: f.items.map((it, i) => i === idx ? { ...it, [key]: value } : it) }));\n  const chooseProduct = (idx: number, productId: string) => { const product = products.find(p => String(p.id) === productId); if (!product) { setForm(f => ({ ...f, items: f.items.map((it, i) => i === idx ? { ...it, productId: '', name: '', price: 0 } : it) })); return; } setForm(f => ({ ...f, items: f.items.map((it, i) => i === idx ? { ...it, productId: String(product.id), name: String(product.title || product.name || ''), price: Number(product.price || 0) } : it) })); };\n  const save = async () => { const isExisting = Boolean(form.id && invoices.some(x => x.id === form.id)); const invoice = { ...form, id: form.id || crypto.randomUUID(), variableSymbol: form.variableSymbol || form.number.replace(/\\D/g, '').slice(-10) }; const res = await fetch('/api/invoices', { method: isExisting ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(invoice) }); if (!res.ok) { alert('Fakturu se nepodařilo uložit do databáze.'); return; } const saved = await res.json(); setInvoices(prev => [saved, ...prev.filter(x => x.id !== saved.id)]); setForm(saved); setSelected(saved); };\n  const remove = async (id: string) => { const res = await fetch(`/api/invoices?id=${encodeURIComponent(id)}`, { method: 'DELETE' }); if (!res.ok) { alert('Fakturu se nepodařilo smazat.'); return; } setInvoices(prev => prev.filter(x => x.id !== id)); if (selected?.id === id) setSelected(null); };"
);
...
if (itemStart !== -1 && itemEnd !== -1) {
  const itemBlock = `        <div><div className="flex items-center justify-between mb-2"><h3 className="font-black">Položky</h3><button onClick={()=>setForm(f=>({...f,items:[...f.items,blankItem()]}))} className="text-xs font-bold rounded-lg border px-3 py-2">+ Přidat položku</button></div><div className="space-y-2">{form.items.map((it,i)=><div key={i} className="grid grid-cols-[minmax(0,1fr)_80px_120px_42px] gap-2 items-center"><select value={it.productId || ''} onChange={e=>chooseProduct(i,e.target.value)} className="rounded-lg border p-2.5 text-sm bg-white"><option value="">{loadingProducts ? 'Načítám produkty…' : 'Vyberte produkt z e-shopu'}</option>{products.map(p=><option key={String(p.id)} value={String(p.id)}>{String(p.title || p.name || 'Produkt')} — {money(Number(p.price || 0))}</option>)}</select><input aria-label="Množství" type="number" min="1" value={it.qty} onChange={e=>setItem(i,'qty',Number(e.target.value))} className="rounded-lg border p-2.5 text-sm"/><div aria-label="Cena produktu" className="rounded-lg border p-2.5 text-sm bg-[#faf8f5] text-right">{money(Number(it.price || 0))}</div><button onClick={()=>setForm(f=>({...f,items:f.items.filter((_,x)=>x!==i)}))} className="rounded-lg border text-red-600 h-10">×</button></div>)}</div></div>\n`;

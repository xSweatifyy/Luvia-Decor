...
export async function sendOrderEmails(order: Order, config: SiteConfig): Promise<SendOrderEmailResult> {
  ...
  const itemsListHtml = order.items.map(item => `
    <tr style="border-bottom: 1px solid #EFEAE3;">
      ...
      <td style="padding: 12px 8px; font-size: 14px; text-align: right; color: #2D2723; font-weight: 600;">${formatMoney(item.price * item.quantity)} Kč</td>
    </tr>
  `).join('');
          ...
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            ...
            <tfoot>
              <tr>
                ...
                <td style="padding: 16px 8px 8px 8px; font-weight: 700; font-size: 18px; text-align: right; color: #8C7355;">${formatMoney(order.totalPrice)} Kč</td>
              </tr>
            </tfoot>
          </table>

          <h3 style="font-size: 16px; font-family: Georgia, serif; margin: 24px 0 12px 0; border-bottom: 2px solid #F0EAE1; padding-bottom: 8px; color: #2D2723;">Kontaktní a doručovací údaje</h3>
    ...
    if (notifyEmail && notifyEmail !== order.customer.email) {
      try {
        await resend.emails.send({
          ...
          to: [notifyEmail],
          subject: `🔔 NOVÁ OBJEDNÁVKA: ${order.orderNumber} (${order.customer.fullName} - ${formatMoney(order.totalPrice)} Kč)`,
          html: emailHtml
        });

...
function toCoupon(row: any) { return { id: row.id, code: row.code, type: row.type, value: Number(row.value), active: row.active, createdAt: row.created_at, note: row.note || '', categoryIds: Array.isArray(row.category_ids) ? row.category_ids.map(String) : [], remainingValue: row.remaining_value == null ? undefined : Number(row.remaining_value) }; }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if ((req.method === 'PUT' || req.method === 'DELETE') && !id) return res.status(400).json({ error: 'Chybí ID slevového kódu.' });
    ...
    if (req.method === 'PUT') {
      ...
      const current = currentRows[0]; const body = req.body || {};
    ...
    if (req.method === 'DELETE') { await sql`DELETE FROM coupons WHERE id = ${id}`; return res.status(200).json({ success: true }); }
    return res.status(405).json({ error: 'Metoda není podporovaná.' });
  } catch (error: any) { console.error('Coupons API error:', error); return res.status(500).json({ error: error?.message || 'Chyba serveru při práci se slevovými kódy.' }); }
}

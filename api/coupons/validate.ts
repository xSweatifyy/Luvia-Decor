...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    await ensureTable();
    const code = String(req.body?.code || '').replace(/\s+/g, ' ').trim().toUpperCase();
    ...
    return send(res, 500, { valid: false, error: 'Slevový kód se nepodařilo ověřit.', details: error?.message || String(error) });
  }
}

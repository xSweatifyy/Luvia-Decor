...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const name = String(req.body?.name || '').trim(); const id = String(req.body?.id || slugify(name));
    ...
    if (req.method === 'PUT') {
      const name = String(req.body?.name || '').trim(); if (!name) return res.status(400).json({error:'Název kategorie je povinný.'});
      const rows = await sql`UPDATE categories SET name=${name}, updated_at=NOW() WHERE id=${id} RETURNING id,name`;
      if (!rows.length) return res.status(404).json({error:'Kategorie nenalezena.'}); return res.status(200).json(rows[0]);
    }

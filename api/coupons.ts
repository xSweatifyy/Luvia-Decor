...
export default async function handler(req:VercelRequest,res:VercelResponse){
 ...
 try{
  ...
  if(req.method==='POST'){
    ...
    if(!code||value<=0)return send(res,400,{error:'Zadejte kód a kladnou hodnotu slevy.'});
    if(type==='percent'&&value>100)return send(res,400,{error:'Procentní sleva může být nejvýše 100 %.'});
    const voucher=note==='gift-voucher';
    const idValue=`cup-${Date.now()}-${Math.random().toString(36).slice(2,7)}`;
  ...
  if(req.method==='PUT'){
    if(typeof req.body?.active==='boolean'){
      const rows=await sql`UPDATE coupons SET active=${req.body.active} WHERE id=${id} RETURNING id,code,type,value,active,created_at,note,category_ids,remaining_value`;
  ...
  return send(res,500,{error:'Slevové kódy se nepodařilo načíst.',details:error instanceof Error?error.message:String(error)});
 }
}

...
const paymentQrUrl = (order: any, size = 260) => {
  const number = String(order.orderNumber || order.id || '');
  const amount = Number(order.totalPrice || 0).toFixed(2);
  return `https://api.paylibo.com/paylibo/generator/czech/image?compress=false&size=${size}&accountNumber=${BANK_ACCOUNT}&bankCode=${BANK_CODE}&amount=${encodeURIComponent(amount)}&currency=CZK&vs=${encodeURIComponent(number)}&message=${encodeURIComponent(number)}`;
};
...
function paymentInfo(order: any) {
  const number = String(order.orderNumber || order.id || '');
  return `<div style="margin-top:26px;padding:22px;background:#fbf8f4;border:1px solid #e3d8ca;text-align:center"><div style="font-size:11px;text-transform:uppercase;letter-spacing:1.7px;color:#a48763;font-weight:700">Platba bankovním převodem</div><h2 style="font-family:Georgia,serif;font-size:22px;margin:8px 0 16px">QR Platba</h2><img src="${paymentQrUrl(order, 260)}" alt="QR Platba – objednávka ${esc(number)}" width="260" height="260" style="display:block;width:260px;height:260px;max-width:100%;margin:0 auto 16px;border:8px solid #fff"/><div style="font-size:14px;line-height:1.9;text-align:left;max-width:360px;margin:auto"><strong>Číslo účtu:</strong> ${BANK_ACCOUNT}/${BANK_CODE}<br><strong>Částka:</strong> ${money(Number(order.totalPrice) || 0)}<br><strong>Variabilní symbol:</strong> ${esc(number)}<br><strong>Poznámka pro příjemce:</strong> ${esc(number)}</div><p style="font-size:12px;color:#817469;line-height:1.6;margin:16px 0 0">QR kód obsahuje účet, částku, variabilní symbol a číslo objednávky jako poznámku pro příjemce.</p></div>`;
}

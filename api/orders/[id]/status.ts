...
import trackingHandler from '../../../src/server/order-tracking';
...
function normalizeOrder(order: any) {
  ...
  const rawItems = Array.isArray(order?.items) ? order.items : Array.isArray(order?.products) ? order.products : Array.isArray(order?.cartItems) ? order.cartItems : [];
  const items = rawItems.map((item: any) => ({ title: firstNonEmpty(item?.title, item?.name, item?.productName, 'Produkt'), quantity: Math.max(1, Number(item?.quantity ?? item?.qty ?? 1) || 1), price: Number(firstNonEmpty(item?.price, item?.unitPrice, item?.pricePerUnit, 0)) || 0, productId: firstNonEmpty(item?.productId, item?.id, '') }));
  const carrier = String(firstNonEmpty(rawDelivery?.carrier, rawShipping?.carrier, order?.shippingCarrier, order?.carrier, order?.deliveryCarrier, '')).trim();
  const shippingPrice = Number(firstNonEmpty(rawDelivery?.price, rawDelivery?.shippingPrice, rawShipping?.price, order?.shippingPrice, typeof order?.shipping === 'number' ? order.shipping : undefined, order?.deliveryPrice, 0)) || 0;
  ...
  return { ...order, items, delivery: { ...rawDelivery, carrier, price: shippingPrice, method, pickupPoint: pickupLabel, pickupPointAddress: pickupAddress }, shippingPrice, shippingCarrier: carrier };
}
...
async function sendCustomerStatusEmail(rawOrder: any, status: OrderStatus) {
  const order = normalizeOrder(rawOrder); const apiKey = process.env.RESEND_API_KEY; const to = String(order?.customer?.email || '').trim();
  if (!apiKey) throw new Error('Chybí RESEND_API_KEY ve Vercel Environment Variables.'); if (!to) throw new Error('Objednávka nemá e-mail zákazníka.');
  const number = String(order?.orderNumber || order?.id || ''); const vs = variableSymbol(number); const delivery = order.delivery || {};
  const deliveryText = delivery.method === 'personal_pickup' ? 'Osobní odběr – Kroměříž' : delivery.method === 'box' ? `BOX – ${delivery.carrier || 'přepravce'}${delivery.pickupPoint ? `: ${delivery.pickupPoint}` : ''}` : delivery.method === 'pickup_point' ? `Výdejní místo – ${delivery.carrier || 'přepravce'}${delivery.pickupPoint ? `: ${delivery.pickupPoint}` : ''}` : delivery.carrier ? `Doručení na adresu – ${delivery.carrier}` : 'Doručení na adresu';
  ...
  const resend = new Resend(apiKey); const result = await resend.emails.send({ from: 'Luvia Decor <objednavky@luvia-decor.cz>', to, replyTo: 'podpora@luvia-decor.cz', subject: `${LABELS[status]} · ${number} | Luvia Decor`, html: layout('Informace k objednávce', body) });
  if (result.error) throw new Error(result.error.message || 'Resend odmítl odeslání e-mailu.'); return { sent: true, id: result.data?.id };
}

export default async function handler(req: VercelRequest, res: VercelResponse) {
  if (String(req.query.action || '') === 'tracking') return trackingHandler(req, res);
  if (req.method !== 'PUT') return res.status(405).json({ error: 'Metoda není podporovaná.' });
  try {
    const requested = String(req.query.id || '').trim(); const status = String(req.body?.status || '') as OrderStatus;
    ...
    const dbId = String(rows[0].id); const order = normalizeOrder({ ...(rows[0].data || {}), id: dbId }); const previous = String(order.status || 'nova') as OrderStatus; const changed = previous !== status;
    const updated: any = { ...order, status, variableSymbol: variableSymbol(order.orderNumber), updatedAt: new Date().toISOString() }; await sql`UPDATE orders SET data = ${JSON.stringify(updated)}::jsonb WHERE id = ${dbId}`;
    let statusEmail: any = { sent: false, skipped: true };
    if (changed || req.body?.resendEmail === true) {
      try { statusEmail = { ...(await sendCustomerStatusEmail(updated, status)), status }; } catch (emailError: any) { statusEmail = { sent: false, status, error: emailError?.message || 'Odeslání e-mailu selhalo.' }; console.error('Status email error:', emailError); }
      updated.statusEmail = statusEmail; updated.resendSent = statusEmail.sent === true; if (statusEmail.error) updated.resendError = statusEmail.error; else delete updated.resendError;

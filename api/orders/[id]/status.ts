...
type OrderStatus = 'nova' | 'zpracovava_se' | 'zaplaceno' | 'odeslano' | 'dokonceno' | 'zruseno';
...
async function sendStatusEmail(order: any, status: OrderStatus) {
  ...
  const result = await resend.emails.send({
    ...
    subject: `${label} ${order.orderNumber} – Luvia Decor`,
    html
  });
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const allowed = Object.prototype.hasOwnProperty.call(STATUS_LABELS, status);

    if (!id) return res.status(400).json({ error: 'Chybí ID objednávky.' });
    if (!allowed) return res.status(400).json({ error: 'Neplatný stav objednávky.' });
    ...
    // E-mail se posílá pouze při skutečné změně stavu, nikoliv při opakovaném uložení stejného stavu.

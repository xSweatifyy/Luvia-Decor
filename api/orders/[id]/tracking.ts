...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const carrier = normalizeCarrier(req.body?.carrier || order.delivery?.carrier);
    const trackingNumber = String(req.body?.trackingNumber || order.tracking?.trackingNumber || '').trim();
      ...
      try {
        if (carrier === 'Zásilkovna') {
          ...
          if (result) {
            ...
            trackingHistory = result.statusText ? [{ ...result, fetchedAt: new Date().toISOString() }, ...trackingHistory].slice(0, 30) : trackingHistory;
          }
        }
      } catch (error: any) {
    ...
    const tracking = {
      ...
      status: String(req.body?.status || existing.status || 'Zásilka evidována'),
      externalStatus,

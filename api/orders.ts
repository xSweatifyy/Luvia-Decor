...
async function getShippingConfig() {
  try {
    ...
    const carriers = Object.fromEntries(Object.entries(saved.carriers || DEFAULT_SHIPPING.carriers).map(([name, cfg]: [string, any]) => [name, {
      enabled: cfg?.enabled !== false,
      address: Math.max(0, Number(cfg?.address ?? 0)),
      pickup_point: Math.max(0, Number(cfg?.pickup_point ?? 0)),
      box: Math.max(0, Number(cfg?.box ?? cfg?.pickup_point ?? 0))
    }]));
    return {
      carriers,
      personalPickup: {
        enabled: saved.personalPickup?.enabled !== false,
        price: Math.max(0, Number(saved.personalPickup?.price ?? 0)),
        label: String(saved.personalPickup?.label || DEFAULT_SHIPPING.personalPickup.label)
      }
    };
  } catch (error) {
    console.warn('Shipping config fallback:', error);
...
function paymentQrUrl(order: any): string {
  ...
  const note = String(order.orderNumber || '');
...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const deliveryMethod = String(delivery?.method || 'address');
    const carrier = String(delivery?.carrier || '');
    if (deliveryMethod === 'personal_pickup') {
      if (!shippingConfig.personalPickup.enabled) return res.status(400).json({ error: 'Osobní odběr momentálně není dostupný.' });
    } else {
      const carrierConfig = shippingConfig.carriers[carrier];
      if (!carrierConfig || carrierConfig.enabled === false) return res.status(400).json({ error: 'Zvolený dopravce není dostupný.' });
      if (!['address', 'pickup_point', 'box'].includes(deliveryMethod)) return res.status(400).json({ error: 'Neplatný způsob doručení.' });
      if ((deliveryMethod === 'pickup_point' || deliveryMethod === 'box') && !String(delivery?.pickupPoint || '').trim()) return res.status(400).json({ error: 'Je nutné vybrat výdejní místo nebo box.' });
    ...
    let subtotal = 0;
    const orderItems = items.map((item: any) => { const price = Number(item.price) || 0; const quantity = Math.max(1, Number(item.quantity) || 1); subtotal += price * quantity; return { productId: item.productId || item.id || 'custom', title: String(item.title || ''), price, quantity, imageUrl: item.imageUrl || '', customNote: item.customNote || '' }; });
    let discount = 0; let appliedCouponCode: string | undefined;
    if (couponCode) { const rows = await sql`SELECT code, type, value FROM coupons WHERE code = ${String(couponCode).trim().toUpperCase()} AND active = TRUE LIMIT 1`; if (rows.length) { appliedCouponCode = rows[0].code; discount = rows[0].type === 'percent' ? Math.round(subtotal * (Number(rows[0].value) / 100)) : Math.min(Number(rows[0].value), subtotal); } }
    ...
    const newOrder: any = {
      ...
      totalPrice: Math.max(0, subtotal - discount + shipping), delivery: normalizedDelivery, status: 'nova', emails: { customer: false, seller: false }
    };
